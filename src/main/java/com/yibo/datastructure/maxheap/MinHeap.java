...
public class MinHeap<E extends Comparable<E>> {
    ...
    /**
     * parent索引元素比左右孩子节点元素大，则交换位置，不断循环
     * @param k
     */
    private void siftDown(int parent){
        ...
        int right = left + 1;
        int min = parent;
        if(left < size && ((E) array[left]).compareTo((E) array[min]) < 0){
            min = left;
        }
        if(right < size && ((E) array[right]).compareTo((E) array[min]) < 0){
            min = right;
        }
        //说明 左右孩子有节点比 parent 节点元素更小
        if(min != parent){
            swap(min, parent);
            //继续递归，进行比对
            siftDown(min);
